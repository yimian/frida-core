...
static const FridaSELinuxRule frida_selinux_rules[] =
{
  ...
  { { "domain", NULL }, "monda_file", "dir", { "search", NULL } },
  { { "domain", NULL }, "monda_file", "fifo_file", { "open", "write", NULL } },
  { { "domain", NULL }, "monda_file", "file", { "open", "read", "getattr", "execute", "?map", NULL } },
  { { "domain", NULL }, "monda_file", "sock_file", { "write", NULL } },
...
frida_selinux_patch_policy (void)
{
  ...
  if (frida_ensure_type (&db, "monda_file", 2, "file_type", "mlstrustedobject", &error) == NULL)
  {
